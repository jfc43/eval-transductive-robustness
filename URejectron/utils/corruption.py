...
def process_one_adv_img(img, severity_range):
...
def gen_corruction_adv_image(batch, severity_level = 5):
    batchs = []

    batch = batch.clone()
    severity_range = np.arange(1, severity_level + 1)
    corruption_imgs_list = Parallel(n_jobs=-1)(delayed(process_one_adv_img)(img, severity_range) for img in batch)
    batchs = torch.stack(corruption_imgs_list, dim=0).transpose(0, 1)

    return batchs


def process_one_img(img, method_name, severity):
    convert_img = trn.Compose([trn.ToPILImage()])
    transform_img = trn.ToTensor()
    ...
    return transform_img(corruption_img)

def gen_corruction_image(batch, method_name, severity_level):
    batch = batch.clone()
    corruption_imgs_list = Parallel(n_jobs=-1)(delayed(process_one_img)(img, method_name, severity_level) for img in batch)
    corruption_batch = torch.stack(corruption_imgs_list, dim=0)

    return corruption_batch
